...
java -jar packr_${PACKR_VERSION}.jar \a
	packr/macos-aarch64-config.json

cp target/filtered-resources/Info.plist native-osx-aarch64/SanLite.app/Contents

echo Setting world execute permissions on launcher
pushd native-osx-aarch64/SanLite.app
chmod g+x,o+x Contents/MacOS/SanLite
popd

codesign -f -s "${SIGNING_IDENTITY}" --entitlements osx/signing.entitlements --options runtime native-osx-aarch64/SanLite.app || true

# create-dmg exits with an error code due to no code signing, but is still okay
create-dmg native-osx-aarch64/SanLite.app native-osx-aarch64/ || true

mv native-osx-aarch64/SanLite\ *.dmg native-osx-aarch64/SanLite-aarch64.dmg

# Notarize app
if xcrun notarytool submit native-osx-aarch64/SanLite-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
    xcrun stapler staple native-osx-aarch64/SanLite-aarch64.dmg
fi
